...
serve(async (req) => {
  ...
  try {
    ...
    if (!Array.isArray(matches)) {
      throw new Error("Invalid input: 'matches' must be an array");
    }
    ...
    const scoredMatches = matches.map((m) => {
      ...
      // Final Reason cleanup (max 10 words)
      const reasonWords = reason.split(" ").slice(0, 10).join(" ");
